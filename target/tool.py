...
from aedi.utility import symlink_directory
...
class ArmGnuToolchainNoneEabiTarget(base.BuildTarget):
    ...
    #     build_script = 'build-baremetal-toolchain.sh'
    ...
    def prepare_source(self, state: BuildState):
        state.source /= 'arm-gnu-toolchain-src-snapshot-14.3.rel1.tar'

    def configure(self, state: BuildState):
        super().configure(state)
        symlink_directory(state.source / 'gnu-devtools-for-arm-v1.5.0', state.build_path)
